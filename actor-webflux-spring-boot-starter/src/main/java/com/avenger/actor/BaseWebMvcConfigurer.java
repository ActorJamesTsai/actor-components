...
import com.avenger.actor.intercept.ReactiveWebFilter;
...
public class BaseWebMvcConfigurer implements WebMvcConfigurer {
    ...
    @Bean
    public FilterRegistrationBean<ReactiveWebFilter> ssoFilterRegistration() {
        FilterRegistrationBean<ReactiveWebFilter> filter = new FilterRegistrationBean<>();
        filter.setName("ssoFilter");
        ReactiveWebFilter reactiveWebFilter = new ReactiveWebFilter(this.exclusions, this.origin);
        reactiveWebFilter.setCorsFlag(this.corsFlag);
        reactiveWebFilter.setMaxAge(this.maxAge);
        filter.setFilter(reactiveWebFilter);
        filter.setOrder(-1);
