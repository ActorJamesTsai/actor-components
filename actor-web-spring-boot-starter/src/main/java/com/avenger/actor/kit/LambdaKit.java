...
public final class LambdaKit {
     ...
     * 包装普通函数（Function）
     * <p>
     * 注：方法签名中的" throws E "编译器会提示多余，但其实是为了将具体的异常向外传递，如果不抛出的话： 1.外层代码中编译器将无法提示有异常需要处理
     * 2.也无法主动在外层捕获具体的异常，如果尝试try一个具体的异常，编译器将提示：在try语句体中永远不会抛出相应异常（Exception 'XXX' is never thrown in the corresponding
     * try block）
     */
     ...
     */
    public static <T, U, E extends Exception> BiPredicate<T, U> wrapBiPredicate(
        BiPredicateWithExceptions<T, U, E> predicate) {
        return (t, u) -> {
            try {
    ...
    /**
     * 如果一个方法绝对不会抛出所申明的异常，可以使用该方法进行包装 如：new String(byteArr, "UTF-8")申明了UnsupportedEncodingException，
     * 但编码"UTF-8"是必定不会抛异常的，所以可以使用uncheck()进行包装 String text = uncheck(() -> new String(byteArr, "UTF-8"))
     */
